...
var SAGE2Connection = {
	...
	*/
	initS2Connection: function(s2Hostname = null, session = null, isMasterDisplayOrUniqueWebpage = true, appIdOfWebview = null) {
		...
		this.isMaster = isMasterDisplayOrUniqueWebpage ? isMasterDisplayOrUniqueWebpage : false; // undefined,null,0 sets false
		var _this = this;
	...
	setupListeners: function() {
		...
		*/
		this.wsio.on("broadcast", function(data) {
			if (data.app === _this.uniqueID) {
				console.log("Broadcast received for function:" + data.func + ", double checking appId:" + data.appId);
				window[data.func](data.data);
			} else {
				console.log("Discarding:" + JSON.stringify(data));
			}
		});
	},
	...
	serverDataGetValue: function(nameOfValue, callback) {
		if (this.isMaster) {
			...
			if (callbackName === undefined) {
				throw "Missing callback for serverDataGetValue. If a function reference was given, change to a string.";
			}
			console.log("Requesting value from server, it should go back to:" + this.uniqueID);
			this.wsio.emit("serverDataGetValue", {
