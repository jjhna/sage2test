...
}
{
	...
	*/
	...
	broadcastData: function() {
		...
		this.serverDataBroadcastSource("testConvert:geoLocationCenter",
			[this.state.center], {
				app: this.id,
				interpretAs: "set",
		...
		// broadcast a different destination to test server handled conversion
		this.serverDataBroadcastSource("testConvert:geoLocationBounds", [{
			source: this.id,
		...
		// name suffix, value, description, callback
		this.serverDataBroadcastDestination("testConvert:viewCenter", [], {
			app: this.id,
			interpretAs: "set",
			dataTypes: ["gps"], // wants these datatypes
			dataFormat: "s2GeoLocation" // should this also describe what is acceptable?
		...
		// name suffix, value, description, callback
		this.serverDataBroadcastDestination("testRange:viewCenter", [], {
			app: this.id,
			interpretAs: "range",
			dataTypes: ["gps"], // wants these datatypes
			dataFormat: "s2GeoLocation" // should this also describe what is acceptable?
		}, "setViewUsingRangeS2GeoLocation");
	},
	...
	replaceMarkerPlotsConvertedByS2: function(value, preventViewChange) {
		...
		// var sLat = null, sLng = null, bLat = null, bLng = null;
		...
		for (let i = 0; i < value.length; i++) {
			...
			// 	sLat = value[i].lat;
			// } else if (value[i].lat > bLat || bLat === null) {
			// 	bLat = value[i].lat;
			// }
			// if (value[i].lng < sLng || sLng === null) {
			// 	sLng = value[i].lng;
			// } else if (value[i].lng > bLng || bLng === null) {
			// 	bLng = value[i].lng;
			// }
		}
	...
	replaceMarkerPlotsHelperFunctionMoveGpsToTop: function(value) {
		...
		// state unable to plot
		if (unableToPlot) {
			console.log("gmapMod> Unable to plot " + e1);
			return false;
		}
		return true;
	},
	...
	setViewUsingRangeS2GeoLocation: function(serverVar) {
		// should be given an array, but conversion is a matter of bound adjustment.
		var useMarkersToHelpCheck = true;

		if (useMarkersToHelpCheck) {
			this.removeAllMarkersFromMap();
		}
		...
		var viewBound = new google.maps.LatLngBounds();
		var coord;
		if (serverVar.length > 1) {
			for (let i = 0; i < serverVar.length; i++) {
				coord = new google.maps.LatLng(serverVar[i].lat, serverVar[i].lng);
				this.addMarkerToMap(serverVar[i]);
				viewBound.extend(coord);
			}
			// TODO maybe modify if need to control zoom level
			this.map.fitBounds(viewBound);
		} else if (serverVar.length === 1) {
			this.setViewUsingS2GeoLocation(serverVar[0]);
		}
	},
