...
var poll_vis = SAGE2_App.extend( {
	init: function(data) {
		...
		// this.votes = [{ name: "A", count: 0 }, { name: "B", count: 0 }, { name: "C", count: 0 }, { name: "D", count: 0 }, { name: "E", count: 0 } ]; 
		this.votes = this.state.votes;//{ A:0, B:0, C:0, D:0, E:0 }; 

	},

	load: function(date) { 
		console.log('poll Load with state value', this.state.value);
		this.refresh(date);
	},
