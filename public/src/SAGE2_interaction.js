		...
		if ($$("login_form_container")) {
						...
						},
						{
							...
							click: function() {
								...
								webix.ajax()
									...
									.catch(function(err) {
										$$("login_error").setValue("Sorry, could not find that user.");
										$$("login_error").show();
									});
							}
						}
					]
				} // end custom user sign in
			];
			...
			if (window.__authStrategy) {
				...
				window.__authStrategy.split(',').forEach(function(field) {
					...
					loginFields.push({
						view: "button",
						label: "Sign in with " + capitalizedField,
						click: function() {
							window.open('/auth/' + field, '_self');
						}
					});
				});
			}
