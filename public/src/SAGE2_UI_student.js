...
function SAGE2_init() {
	...
	window.addEventListener('message', function(event) {
		...
		if (event.data.cmd === "openlink") {
			...
			var dropPartitionCenterX = 0;
			var dropPartitionCenterY = 0; 
			if( dropPartitionId != "null" ){
				dropPartitionCenterX = displayUI.partitions[dropPartitionId].left + displayUI.partitions[dropPartitionId].width/2.0; 
				dropPartitionCenterY = displayUI.partitions[dropPartitionId].top + displayUI.partitions[dropPartitionId].height/2.0; 
			}

			var converted = displayUI.scaleScreenCoords(dropPartitionCenterX, dropPartitionCenterY); 
...
function setupListeners() {
	...
	wsio.on('deletePartitionWindow', function(data) {
		if( data.id == dropPartitionId ){
			dropPartitionId = "null"; 
			interactor.dropPartitionId  = "null"; 
		}
	...
	wsio.on('partitionMoveAndResizeFinished', function(data) {	
		...
		if( data.id == dropPartitionId ){
			var dropPartitionCenterX = displayUI.partitions[dropPartitionId].left + displayUI.partitions[dropPartitionId].width/2.0; 
			var dropPartitionCenterY = displayUI.partitions[dropPartitionId].top + displayUI.partitions[dropPartitionId].height/2.0; 

			var converted = displayUI.scaleScreenCoords(dropPartitionCenterX, dropPartitionCenterY); 
			interactor.dropPartitionX = dropPartitionCenterX;//converted.x;
			interactor.dropPartitionY = dropPartitionCenterY;//converted.y;
		}
	});
	...
	wsio.on('updateDropPartition', function(data) {
		dropPartitionId = data.id; 
		interactor.dropPartitionId = data.id;
		var dropPartitionCenterX = displayUI.partitions[dropPartitionId].left + displayUI.partitions[dropPartitionId].width/2.0; 
		var dropPartitionCenterY = displayUI.partitions[dropPartitionId].top + displayUI.partitions[dropPartitionId].height/2.0; 
		...
		//console.log("update drop partition " + dropPartitionId);
	});
...
function fileUploadFromUI() {
	...
	interactor.uploadFiles(thefile.files, converted.x, converted.y); //change from 0,0
}
...
function handleClick(element) {
	...
	if (element.id === "sage2pointer"        || element.id === "sage2pointerContainer" || element.id === "sage2pointerLabel") {
	...
	} else if(element.id === "quickShareMediaContainer" || element.id === "quickShareMedia" || element.id === "quickShareMediaLabel"){
		//showDialog("quickShareMediaDialog");
		//showDialog("mediaBrowserDialog");
	...
	} else if(element.id === "presentation"|| element.id === "presentationContainer" || element.id === "presentationLabel") {
		showDialog('presentationDialog');
	}else if (element.id === "settings"    || element.id === "settingsContainer"    || element.id === "settingsLabel") {
		showDialog('settingsDialog');
	}else if (element.id === "poll"    || element.id === "pollContainer"    || element.id === "pollLabel") {
		showDialog('pollDialog');
		...
		var converted = displayUI.scaleScreenCoords(dropPartitionCenterX, dropPartitionCenterY); 
		data.xLaunch = converted.x;
		data.yLaunch = converted.y;
		...
		*/
	...
	} else if(element.id == "quickShareUploadButton"){
	...
	} else if (element.id === "presentationCloseBtn") {
		
		hideDialog('presentationDialog');
	...
	} else if (element.id === "pollCloseBtn") {
		// Settings Dialog
		hideDialog('pollDialog');
	} else if (element.id === "pollSubmitBtn") {
		voteInPoll();
		// Settings Dialog
		hideDialog('pollDialog');
	} 
	...
	} else if (element.id === "presExportDragNDrop") { //PRESENTATION
		// Remove all the running applications
		// wsio.emit('clearDisplay');
		showDialog('localfileDialog');
		hideDialog('presentationDialog');
	} else if (element.id === "presentationScreencast") { //PRESENTATION
		// Layout the applications
		interactor.startScreenShare();
		// wsio.emit('tileApplications');
		hideDialog('presentationDialog');
	} else if (element.id === "streamPowerpoint") { //PRESENTATION
		...
		webix.ui({
			...
			modal: true,
			zIndex: 9999,
			head: "Show your presentation through the web",
			width: 400,
			body: {
				view: "form",
				borderless: false,
				elements: [
					{view: "label", value: "", id: "1", label: "1. In powerpoint, go to Slide Show menu", name: "step1"},
					{view: "label", value: "", id: "2", label: "2. Select 'broadcast slide show' ", name: "step2"},
					{view: "label", value: "", id: "3", label: "3. Follow the instructions in powerpoint", name: "step3"},
					{view: "text", value: "", id: "browser_url", label: "Please enter the URL provided by powerpoint for your talk:", name: "browser_url"},
					// {view: "text", value: "", id: "browser_search", label: "or search terms:", name: "browser_search"},
					{margin: 5, cols: [
						{view: "button", value: "Cancel", click: function() {
							this.getTopParentView().hide();
...
// Build a webix dialog
		webix.ui({
			...
			body: {
				...
				borderless: false,
				elements: [
					{view: "label", value: "", id: "1", label: "Go to google docs and share your presentation.  Copy the sharable link.", name: "1"},
					{view: "text", value: "", id: "browser_url", label: "Enter the shared url for your Google slides:", name: "browser_url"},
					// {view: "text", value: "", id: "browser_search", label: "or search terms:", name: "browser_search"},
					{margin: 5, cols: [
						{view: "button", value: "Cancel", click: function() {
							this.getTopParentView().hide();
...
function voteInPoll(){
	...
	for (var i = 0, length = radios.length; i < length; i++) {
        ...
        // do whatever you want with the checked radio
        	vote[radios[i].value] = 1; 
        // only one radio can be logically checked, don't check the rest
       	 break;
    	}
	}	
	console.log(vote);
	wsio.emit('voteInPoll', vote);
}
...
function uiDrawZoneRemoveSelfAsClient() {
	...
	var converted = displayUI.scaleScreenCoords(dropPartitionCenterX, dropPartitionCenterY); 
	dataForApp.xLaunch = converted.x;
	dataForApp.yLaunch = converted.y;
