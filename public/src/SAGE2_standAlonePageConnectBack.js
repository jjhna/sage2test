...
var SAGE2Connection = {
	...
	*/
	initS2Connection: function(s2Hostname = null, session = null, isMasterDisplayOrUniqueWebpage = true, appIdOfWebview = null) {
		console.log("Attempting to connect to SAGE2 server");
		// if not given a hostname, probably this is an app page connecting back.
		if (s2Hostname == null) {
			// first check if url was given parameters
			this.detectConnectionInformationFromBrowser(); // Possible to fail finding appId, pointerName, pointerColor
			session = this.getCookie("session");
		} else {
			console.log("Using provided info");
	...
	setupListeners: function() {
		...
		this.wsio.on("sendDataToClient", function(data) {
			try {
				...
				console.log(e);
				if (window[data.func] === undefined) {
					console.log("Most likely data.func, " + data.func + ", doesn't exist within the window");
				}
			}
		});
		...
		if (address.indexOf("?") !== -1) {
			...
			for (var i = 0; i < pairs.length; i++) {
				...
				if (onePair[0] == "appId") {
					...
					console.log("-- Using appId from URL parameter:" + this.appId);
				} else if (onePair[0] == "pointerName") {
					this.pointerName = onePair[1];
					console.log("-- Using pointerName from URL parameter:" + this.pointerName);
				} else if (onePair[0] == "pointerColor") {
					this.pointerColor = onePair[1];
					console.log("-- Using pointerColor from URL parameter:" + this.pointerColor);
				}
			}
		}
		// If URL params didn't contain pointer name, might be in localStorage
		if (!this.pointerName && localStorage.SAGE2_ptrName) {
			this.pointerName = localStorage.SAGE2_ptrName;
			console.log("-- Using pointerName from localStorage:" + this.pointerName);
		}
		if (!this.pointerColor && localStorage.SAGE2_ptrColor) {
			this.pointerColor = localStorage.SAGE2_ptrColor;
			console.log("-- Using pointerColor from localStorage:" + this.pointerColor);
		}
		// Or cookies
		if (!this.pointerName && this.getCookie("SAGE2_ptrName")) {
			this.pointerName = this.getCookie("SAGE2_ptrName");
			console.log("-- Using pointerName from cookie:" + this.pointerName);
		}
		if (!this.pointerColor && this.getCookie("SAGE2_ptrColor")) {
			this.pointerColor = this.getCookie("SAGE2_ptrColor");
			console.log("-- Using pointerColor from cookie:" + this.pointerColor);
		}
		...
		*/
		this.open = function(callback) {
			...
			this.ws.onclose = function(evt) {
				...
				alert("Connection to server lost. To try reconnect, please refresh the page.");
			};
		};
